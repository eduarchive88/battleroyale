...
const App: React.FC = () => {
  ...
  const createRoom = () => {
    ...
    network.init(finalCode, true, (state) => {
      setGameState(state);
      ...
      <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-[#020617] text-white bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')]">
        ...
        <div className="w-full max-w-md p-10 bg-slate-900/80 backdrop-blur-xl rounded-[3rem] border-2 border-blue-500/30 shadow-[0_0_50px_rgba(59,130,246,0.2)] space-y-8">
          <div className="space-y-4">
            ...
            <button onClick={joinRoom} disabled={isConnecting} className={`w-full py-5 bg-blue-600 hover:bg-blue-500 rounded-2xl font-black text-2xl shadow-lg transition-transform active:scale-95 ${isConnecting ? 'opacity-50' : ''}`}>
              {isConnecting ? '전장 진입 중...' : '입장하기'}
            </button>
          </div>
        ...
        <div className="grid grid-cols-12 gap-10">
          <div className="col-span-5 space-y-6">
            <div className="bg-slate-900 p-8 rounded-[3rem] border border-blue-500/20 shadow-2xl space-y-4">
              ...
              <button onClick={() => { if(newQuiz.question) { setQuizList([...quizList, newQuiz]); setNewQuiz({question:'', options:['','','',''], answer:0}); } }} className="w-full py-4 bg-blue-600 rounded-2xl font-black hover:bg-blue-500 transition-colors">퀴즈 추가</button>
            </div>
            <div className="bg-slate-900 p-8 rounded-[3rem] border border-emerald-500/20 shadow-2xl space-y-4 text-center">
              ...
              <button onClick={downloadCSVTemplate} className="text-xs text-emerald-500/70 hover:text-emerald-500 underline decoration-dotted">샘플 양식 다운로드</button>
            </div>
          </div>
          <div className="col-span-7 space-y-8">
            ...
            <div className="flex gap-4">
               <input className="flex-1 p-4 bg-slate-900 border border-white/10 rounded-2xl text-center text-2xl font-black uppercase tracking-widest outline-none focus:ring-2 ring-blue-500" placeholder="방 코드 (예: ROOM7)" value={customCode} onChange={e => setCustomCode(e.target.value)} />
               <button onClick={createRoom} disabled={isConnecting} className="px-10 py-6 bg-blue-600 hover:bg-blue-500 rounded-2xl font-black text-2xl shadow-xl transition-all">
                 {isConnecting ? '생성 중...' : '전장 생성'}
               </button>
            </div>
          </div>
        </div>
      ...
      <div className="h-screen bg-[#020617] text-white flex flex-col p-10">
        <header className="flex justify-between items-center mb-8 bg-slate-900/50 p-8 rounded-[3rem] border border-white/10 shadow-2xl">
          ...
          <div className="flex flex-col items-end gap-3">
             <p className="text-slate-500 font-bold">참여 영웅: {players.length}명</p>
             <button onClick={() => {
            ...
            return (
              <div key={tId} className={`bg-slate-900/80 p-6 rounded-[2.5rem] border transition-all ${teamPlayers.length > 0 ? 'border-blue-500/50 shadow-xl' : 'border-white/5 opacity-50'}`}>
                ...
                <div className="space-y-3 text-sm">
                  <div className={`p-3 rounded-xl flex justify-between font-bold ${qUser ? 'bg-blue-600/20 text-blue-400' : 'bg-white/5 text-slate-600'}`}><span>🧠 {qUser?.name || '문제풀이'}</span><span>{qUser ? '완료' : '대기'}</span></div>
                  ...
                  <div className={`p-3 rounded-xl flex justify-between font-bold ${cUser ? 'bg-red-600/20 text-red-400' : 'bg-white/5 text-slate-600'}`}><span>⚔️ {cUser ? `${cUser.name}(${cUser.classType})` : '전투요원'}</span><span>{cUser ? '완료' : '대기'}</span></div>
                </div>
              </div>
            );
          })}
  ...
  if (view === 'lobby') {
    ...
    return (
      <div className="h-screen bg-[#020617] text-white flex flex-col p-6 overflow-hidden">
        ...
        {!gameState.roomCode && <div className="text-center font-bold text-yellow-500 animate-pulse">호스트로부터 전장 정보를 불러오는 중...</div>}
                ...
                <div key={tId} className={`p-8 rounded-[3.5rem] border-2 transition-all ${isMyTeam ? 'bg-blue-600/20 border-blue-500 shadow-2xl' : 'bg-slate-900 border-white/5'}`}>
                  ...
                  <div className="space-y-3">
                    <button disabled={quizTaken || !!myPlayer} onClick={() => setPendingSelection({ teamId: tId.toString(), role: Role.QUIZ })} className={`w-full p-4 rounded-2xl text-left font-black flex justify-between transition-all ${pendingSelection?.teamId === tId.toString() && pendingSelection?.role === Role.QUIZ ? 'ring-4 ring-white bg-blue-600' : quizTaken ? 'bg-slate-950 opacity-40 grayscale' : 'bg-slate-800 hover:bg-slate-700'}`}>
                      <span>🧠 문제풀이</span><span className="text-xs">{quizTaken ? '점유됨' : '선택 가능'}</span>
                    </button>
                    <button disabled={supporters >= 2 || !!myPlayer} onClick={() => setPendingSelection({ teamId: tId.toString(), role: Role.SUPPORT })} className={`w-full p-4 rounded-2xl text-left font-black flex justify-between transition-all ${pendingSelection?.teamId === tId.toString() && pendingSelection?.role === Role.SUPPORT ? 'ring-4 ring-white bg-emerald-600' : supporters >= 2 ? 'bg-slate-950 opacity-40 grayscale' : 'bg-slate-800 hover:bg-slate-700'}`}>
                      <span>🛡️ 서포터 ({supporters}/2)</span><span className="text-xs">{supporters >= 2 ? '점유됨' : '선택 가능'}</span>
                    </button>
        ...
        {myPlayer && <div className="fixed bottom-12 left-0 right-0 text-center font-black text-blue-400 animate-pulse text-2xl italic">다른 영웅들을 기다리는 중...</div>}
      </div>
    );
  }
  ...
  if (view === 'game' && myPlayer) {
    ...
    const currentQuiz = gameState.quizzes[gameState.currentQuizIndex] || { question: '문제가 준비되지 않았습니다.', options: ['대기'], answer: 0 };
